...
const MyPage = () => {
  ...
  const handleAddressEditClick = () => setShowAddressForm(true);
  ...
  return (
    <Container className="py-5">
          ...
          <Card>
            <Card.Body className="p-0">
              <div className="list-group list-group-flush">
                <button className={`list-group-item list-group-item-action d-flex align-items-center ${activeTab === "info" ? "active" : ""}`} onClick={() => setActiveTab("info")}>
                  <FaUser className="me-2" /> 개인정보
                </button>
                <button className={`list-group-item list-group-item-action d-flex align-items-center ${activeTab === "address" ? "active" : ""}`} onClick={() => setActiveTab("address")}>
                  <FaMapMarkerAlt className="me-2" /> 배송지 관리
                </button>
                <button className={`list-group-item list-group-item-action d-flex align-items-center ${activeTab === "orders" ? "active" : ""}`} onClick={() => setActiveTab("orders")}>
                  <FaShoppingBag className="me-2" /> 주문내역
                </button>
              </div>
            </Card.Body>
          </Card>
            ...
            <Card>
              <Card.Header className="d-flex justify-content-between align-items-center">
                ...
                <Button variant="outline-primary" size="sm" onClick={handleEditClick}>
                  <FaEdit className="me-1" /> {userInfo ? "수정" : "로그인하여 수정"}
                </Button>
              </Card.Header>
              <Card.Body>
                {userInfo ? (
                  ...
                  <div className="text-center py-4">
                    ...
                    <Button variant="primary" onClick={() => navigate('/login', { state: { from: '/mypage' } })}>
                      <FaSignInAlt className="me-2" /> 로그인하기
                    </Button>
                  </div>
                )}
              </Card.Body>
          ...
          {activeTab === "address" && (
            <Card>
              <Card.Header className="d-flex justify-content-between align-items-center">
                ...
                <Button variant="outline-primary" size="sm" onClick={handleAddressEditClick}>
                  <FaEdit className="me-1" /> {userInfo ? "수정" : "로그인하여 수정"}
                </Button>
              </Card.Header>
              <Card.Body>
      ...
      {showUpdateForm && (
        <MyPageUpdate user={userInfo} field="info" onUpdate={handleUpdateSuccess} onClose={() => setShowUpdateForm(false)} />
      )}

      {showAddressForm && (
        <AddressUpdate user={userInfo} onUpdate={handleAddressUpdateSuccess} onClose={() => setShowAddressForm(false)} />
      )}
    </Container>
  );
};
